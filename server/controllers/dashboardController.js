...
exports.getDonorDashboard = async (req, res) => {
  try {
    ...
    const activeDonations = await Donation.countDocuments({ donor: userId, status: { $in: ['Available', 'Assigned', 'PickupScheduled'] } });
    const completedDonations = await Donation.countDocuments({ donor: userId, status: 'Delivered' });

    const deliveredDonations = await Donation.find({ donor: userId, status: 'Delivered' });
    const foodDonated = deliveredDonations.reduce((sum, d) => sum + (d.quantity || 0), 0);
    const mealsProvided = Math.round(foodDonated * 4); // ~4 meals per kg estimate
...
exports.getNGODashboard = async (req, res) => {
  try {
    ...
    const recentDonations = await Donation.find({ status: 'Available' })
      .sort({ createdAt: -1 })
      .limit(10)
      .populate('donor', 'name');
